...
#define TUPI_TILE_ANIMADO  0x08  // tile tem animação por frames

// --- Structs ---

// Posição de um frame no atlas (coluna, linha em tiles)
typedef struct {
    int coluna;
    int linha;
} TupiTileFrame;
...
typedef struct {
    ...
    uint8_t _pad[3];
    float   tempo;     // acumulador de animação
...
// Implementação Rust (linkada via libtupi_seguro)
