...
void tupi_mapa_desenhar(TupiMapa *mapa, int z_base) {
    ...
    for (int lin = 0; lin < mapa->linhas; lin++) {
        for (int col = 0; col < mapa->colunas; col++) {
            ...
            /* Arredonda para pixel inteiro antes de enviar ao batch.
               Sem isso, posições sub-pixel vindas da câmera em tela
               cheia/letterbox causam bleeding (pixel branco) na borda
               inferior/lateral do tile, porque o sampler GL_NEAREST
               amostra o tile adjacente.                                */
            ...
            // UVs exatas sem inset: o atlas usa GL_NEAREST (sem filtragem
            // linear), portanto nao ha texture bleeding e o inset so
            // removeria pixels reais do tile.
            float u0  = ((float)fr->coluna * sw) / aw;
            float v0  = ((float)fr->linha  * sh) / ah;
            float u1  = u0 + sw / aw;
            float v1  = v0 + sh / ah;
