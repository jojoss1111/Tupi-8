...
local Cenas = {}
local _lista = {}; local _atual = nil; local _pilha = {}; local _fade = nil
...
function Cenas.nova(nome)
    assert(type(nome)=="string"); local cena = {}; _lista[nome] = cena
    return setmetatable({ _cena=cena }, Builder)
end
function Cenas.registrar(nome, tabela)
    assert(type(nome)=="string" and type(tabela)=="table"); _lista[nome] = tabela
end

function Cenas.atual()      return _atual end
function Cenas.nomeAtual()
    for nome, cena in pairs(_lista) do if cena == _atual then return nome end end
end
function Cenas.existe(nome)      return _lista[nome] ~= nil end
function Cenas.profundidade()    return #_pilha end

local function _chamar(cena, metodo, ...)
    if cena and cena[metodo] then
        local ok, err = pcall(cena[metodo], ...)
        if not ok then error(("[Cenas] Erro em %s(): %s"):format(metodo, tostring(err)), 2) end
    end
end
local function _sair_atual()  _chamar(_atual, "sair") end
local function _entrar(nome, ...)
    local cena = _lista[nome]
    if not cena then error(("[Cenas] Cena não encontrada: '%s'"):format(nome)) end
    _atual = cena; _chamar(_atual, "init", ...)
end

function Cenas.trocar(nome, ...)
    assert(type(nome)=="string"); _pilha = {}; _sair_atual(); _entrar(nome, ...)
end
function Cenas.trocarComFade(nome, duracao, ...)
    assert(type(nome)=="string"); if _fade then return end
    _fade = { duracao=duracao or 0.4, tempo=0.0, fase="saindo", proximo=nome, args={...} }
end
function Cenas.empurrar(nome, ...)
    assert(type(nome)=="string" and _lista[nome])
    table.insert(_pilha, _atual); _atual = nil; _entrar(nome, ...)
end
function Cenas.desempilhar()
    assert(#_pilha > 0, "[Cenas] pilha vazia"); _sair_atual(); _atual = table.remove(_pilha)
end
function Cenas.substituirTopo(nome, ...)
    assert(type(nome)=="string" and _lista[nome]); _sair_atual(); _atual = nil; _entrar(nome, ...)
end
function Cenas.update()
    local dt = Cenas._getDt and Cenas._getDt() or 0
    if _fade then
        local f = _fade; f.tempo = f.tempo + dt
        if f.fase == "saindo" then
            if f.tempo >= f.duracao * 0.5 then
                _pilha={}; _sair_atual(); _entrar(f.proximo, table.unpack(f.args))
                f.fase="entrando"; f.tempo=0.0
            end
        elseif f.fase == "entrando" then
            if f.tempo >= f.duracao * 0.5 then _fade = nil end
        end
    end
...
function Cenas.draw()
    _chamar(_atual, "draw")
    if _fade then
        local f = _fade; local metade = f.duracao * 0.5; local alpha
        if f.fase == "saindo" then alpha = math.min(f.tempo/metade, 1.0)
        else alpha = math.max(1.0-(f.tempo/metade), 0.0) end
        local larg, alt = Cenas._getDimensoes and Cenas._getDimensoes() or 800, 600
        C.tupi_cor(0,0,0,alpha); C.tupi_retangulo(0,0,larg,alt); C.tupi_cor(1,1,1,1)
    end
end
...
function InstMeta:criarMapa(lm, am, lt, at)
    assert(lm>0 and am>0 and lt>0 and at>0,
        "[Mapa] criarMapa: argumentos inválidos")
...
function InstMeta:flags(tile_id, opts)
    ...
    if not d then
        ...
        local id0 = tile_id - 1
        local col = math.floor(id0 / 1000)
        local row = id0 % 1000
        d = {
            frames = {{coluna=col, linha=row}},
...
function InstMeta:frames(tile_id, frames_t, fps, loop)
    assert(type(tile_id)=="number" and tile_id >= 1,
        "[Mapa] frames: tile_id inválido")
    assert(type(frames_t)=="table" and #frames_t > 0,
        "[Mapa] frames: lista de frames vazia")
    local d = self._defs[tile_id]
    if not d then
        self:flags(tile_id, {})
...
function InstMeta:carregarArray(arr)
    assert(type(arr)=="table" and #arr > 0,
        "[Mapa] carregarArray: array vazio")
    self._array = arr
end
...
function InstMeta:build()
    ...
    for tid in pairs(usados) do
        _garantirDef(self, tid)
    end

    self._atlas_spr = C.tupi_sprite_carregar(self._atlas_path)
    assert(self._atlas_spr ~= nil,
        "[Mapa] falha ao carregar atlas: "..self._atlas_path)
...
function InstMeta:desenhar(z)
    if self._valido then C.tupi_mapa_desenhar(self._mapa_c, z or 0) end
end
