...
-- Tela cheia; letterbox=true mantém aspect ratio com barras pretas
...
-- Aplica uma cor e retorna função para resetar para branco
...
-- Tempo mínimo (s) para teclaSegurando() retornar true
...
-- true enquanto segurada (respeita tempo mínimo configurado)
...
function Input.mouseX()   return tonumber(C.tupi_mouse_mundo_x())  end -- coordenada mundo
...
local function _info(c) -- converte TupiColisao C para tabela Lua
...
-- Cria corpo dinâmico com massa, elasticidade e atrito
...
-- Corpo imóvel (massa=0, sem elasticidade)
...
-- Retorna hitbox retangular/circular a partir da posição do corpo
...
-- Resolve colisão entre dois corpos dinâmicos ou entre corpo e objeto estático
...
-- Copia posição do corpo para o wrapper de sprite
