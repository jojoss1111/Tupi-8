...
local BASE_W = 160
local BASE_H = 144
local REPETE_ATRASO = 0.28
local REPETE_PASSO  = 0.04
local INDENT = "  "
...
-- Template padrão usa os três callbacks PT-BR
...
-- Símbolos e números agora vêm do módulo KB compartilhado.
-- Para trocar o layout: no console, digite  teclado abnt2  ou  teclado us
...
function Editor:_capturarTexto(dt)
    ...
    -- Nova função auxiliar para tentar compor acentos
    local function tentarInserir(char_digitado)
        if self._acentoPendente then
            local combinado = KB.combinar(self._acentoPendente, char_digitado)
            if combinado then
                self:_inserirChar(combinado) -- Sucesso: combinou '~' + 'a' = 'ã'
            else
                -- Não combinou: ex: '~' + 'x'. Insere o acento e depois a letra solta
            ...
            qualquer = true
        elseif KB.ehAcento(char_digitado) then
            self._acentoPendente = char_digitado -- Segura a "dead key"
        else
            self:_inserirChar(char_digitado) -- Letra normal
            qualquer = true
        end
    end
