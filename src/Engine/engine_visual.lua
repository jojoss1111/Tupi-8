...
local function _enviarComEspelho(wrapper, camada)
    ...
    -- UV base sem inset
    local base_u0 = (col * cw) / lw;  local base_v0 = (lin * ch) / lh
    local base_u1 = base_u0 + cw / lw; local base_v1 = base_v0 + ch / lh

    -- Aplica inset de meio texel apenas nas bordas internas do sprite sheet
    -- (igual ao que _calcular_uv_objeto faz no lado C em Sprites.c)
    local tem_viz_esq = col > 0
    local tem_viz_top = lin > 0
    local tem_viz_dir = (col + 1) * cw < lw
    local tem_viz_bot = (lin + 1) * ch < lh

    -- MUDE AQUI: Remova o 0.5 e deixe 0.0
    local texel_w = 0.0 -- Antes: 0.5 / lw
    local texel_h = 0.0 -- Antes: 0.5 / lh

    local u0 = base_u0 + (tem_viz_esq and texel_w or 0.0)
    local v0 = base_v0 + (tem_viz_top and texel_h or 0.0)
    local u1 = base_u1 - (tem_viz_dir and texel_w or 0.0)
    local v1 = base_v1 - (tem_viz_bot and texel_h or 0.0)

    if e.h then u0, u1 = u1, u0 end
    if e.v then v0, v1 = v1, v0 end

    local sw = cw * tonumber(obj.escala); local sh = ch * tonumber(obj.escala)

    local x0 = math.floor(tonumber(obj.x) + 0.5); local y0 = math.floor(tonumber(obj.y) + 0.5)
    local x1 = x0 + sw;                            local y1 = y0 + sh
...
local function _uv_char(f, codigo)
    ...
    local col = idx % f.colunas; local lin = math.floor(idx / f.colunas)
    local u0 = col * f.uv_px_x; local v0 = lin * f.uv_px_y
    return u0, v0, u0 + f.uv_px_x, v0 + f.uv_px_y
end
...
function Texto.desenhar(x, y, z, texto, escala, transparencia, fonte)
    ...
    for i = 1, #texto do
        ...
        if cod == 10 then cx = x; y = y + sh
        else _enviar_glifo(fonte, cod, cx, y, sw, sh, transparencia, z); cx = cx + sw end
    end
end
...
function Texto.desenharCaixa(x, y, z, largura, altura, texto, escala, transp, fonte, frame, tamTile, escBorda, transpBorda, recuo)
    ...
    local function uv(col,lin) local u0=col*uvTW;local v0=lin*uvTH;return u0,v0,u0+uvTW,v0+uvTH end
