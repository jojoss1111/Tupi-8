...
-- tabela de conversão UTF-8 para Latin-1
...
-- mapa extra para caracteres fora do Latin-1 padrão
M.MAPA_EXTRA = {}

-- retorna true se o byte é continuação UTF-8
local function _cont(b)
    return b and b >= 0x80 and b <= 0xBF
end

-- retorna o tamanho em bytes do caractere UTF-8 na posição i
...
-- iterador que percorre a string caractere a caractere (respeitando UTF-8)
...
-- igual ao limpar, mas também usa MAPA_EXTRA antes de descartar
...
-- monkey-patch em Tupi.texto para normalizar strings antes de desenhar
