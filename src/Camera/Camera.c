...
// Eixo Y cresce para BAIXO (convenção SDL2/tela).
...
// Recalcula _cam_x/_cam_y a partir do alvo + âncora.
// âncora = ponto na tela onde o alvo aparece; -1 usa o centro.
// roundf() = pixel-snapping, evita bleeding nos tiles.
...
// View matrix 4×4 column-major para SDL2 (Y↓).
// SDL2 não inverte Y, então a translação é simplesmente -cam.
// Sem isso o mapa rolava na direção errada ao mover o alvo.
static TupiMatriz _view_matrix(const TupiCamera* cam) {
    float co = cosf(-cam->rotacao) * cam->zoom;
    float si = sinf(-cam->rotacao) * cam->zoom;

    // Pixel-snap garante translação em pixel inteiro mesmo com zoom/rotação
    float cx = roundf(cam->_cam_x);
    float cy = roundf(cam->_cam_y);

    // SDL2: Y↓ → translação direta, sem negar cy
...
TupiCamera tupi_camera_criar(float alvo_x, float alvo_y,
    ...
    cam.alvo_x   = alvo_x;
    cam.alvo_y   = alvo_y;
    cam.ancora_x = ancora_x; // -1 = centro da janela
...
// --- Frame (chamado pelo Renderer a cada frame) ---
...
// Segue o alvo suavemente — lerp exponencial, independente de frame rate
...
void tupi_camera_tela_para_mundo(const TupiCamera* cam, int largura, int altura,
    ...
    // Origem da tela = canto superior-esquerdo (SDL2)
...
// --- Wrappers para Lua (usam câmera ativa) ---
