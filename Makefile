...
.PHONY: all menu sdl2 win dist-linux rodar compilar_rust compilar_rust_win shaders \
...
dist-linux: compilar_rust shaders $(DIST_OBJS)
...
limpar:
	...
	@rm -rf .build $(SDL2_LIB) $(WIN_LIB) $(DIST_BIN_LINUX) $(SHADERS_SPV) $(SHADERS_EMBED)
...
instalar-deps-linux:
	...
	@printf "  sdl2  vulkan-icd-loader  shaderc  luajit  base-devel  rust\n\n"
	@printf "  > Instalar agora? [s/N]: " && read CONF; \
	if [ "$$CONF" = "s" ] || [ "$$CONF" = "S" ]; then \
		sudo pacman -S --needed sdl2 vulkan-icd-loader shaderc luajit base-devel rust \
